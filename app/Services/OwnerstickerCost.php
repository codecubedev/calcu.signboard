...
class OwnerstickerCost
{
    ...
    public function owncalculate(array $data): float
    {
        ...
        $height = (float) $data['ownHeight'];
        $width = (float) $data['ownWidth'];
        $pcs = (int) ($data['pcs'] ?? 1);
        ...
        $printprice = isset($data['ownPaintInputs'])
            ? (float) $data['ownPaintInputs']
            : 7.50;

        $paintCost = !empty($data['usePaint']) ? $area * $printprice : 0;

        $orcalprice = isset($data['ownOracalInputs'])
        ...
        $printprice = !empty($data['ownPaintInputs']) ? (float) $data['ownPaintInputs'] : 7.50;
        ...
        foreach ($data['lightingTypes'] as $lighting) {
            if (!empty($data['ownLightingDetails'])) {
                $price = (float) $data['ownLightingDetails'];
            } else {
