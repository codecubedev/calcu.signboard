...
class BusinessCost
{
    ...
    //             } elseif (strpos($material, 'neon') !== false || strpos($material, 'clear acrylic') !== false) {
    ...
    // }
        ...
        $height = (float) $data['busHeight'];
        $width = (float) $data['busWidth'];
        $pcs = (int) ($data['pcs'] ?? 1);
        ...
        $printprice = isset($data['busPaintInputs'])
            ? (float) $data['busPaintInputs']
            : 7.50;

        $paintCost = !empty($data['usePaint']) ? $area * $printprice : 0;

        $orcalprice = isset($data['busOracalInputs'])
        ...
        $printprice = !empty($data['busPaintInputs']) ? (float) $data['busPaintInputs'] : 7.50;
        ...
        foreach ($data['lightingTypes'] as $lighting) {
            if (!empty($data['busLightingDetails'])) {
                $price = (float) $data['busLightingDetails'];
            } else {
